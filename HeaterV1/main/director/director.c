...
// Update scroll window based on current selected index. Adjusts pad_top so the
// selected row stays within [window_start_idx, window_end_idx]. Each scroll step is 23px.
static void settings_scroll_sync(void) {
    if (!s_scroll_column) {
        // Attempt lazy lookup if not cached yet (page might have just loaded)
    ...
    // Expand window if selection moved above current window
    if (sel < window_start_idx) {
        window_start_idx = sel;
    }
    // Expand window if selection moved beyond end
    if (sel > window_end_idx) {
        window_start_idx = sel - (WINDOW_COUNT - 1);
        if (window_start_idx < 0) window_start_idx = 0;
    }
    // Ensure window does not exceed maximum possible start
    int max_start = SETTINGS_COUNT - WINDOW_COUNT;
    if (max_start < 0) max_start = 0; // fewer items than window
        ...
        switch (msg.btn_id) {
            ...
            case BUTTON_RIGHT_BOTTOM: { // RIGHT_BOTTOM
                if (s_settings_edit_mode) {
                    ...
                    // navigate down
                    if (activeSetting < SETTINGS_COUNT - 1) {
                        int idx = activeSetting + 1;
                        s_settings_selected_idx = idx;
                        lv_subject_set_int(&settingsSelect, idx);
                        settings_scroll_sync();
            ...
            case BUTTON_RIGHT_CENTER: { // RIGHT_CENTER -> toggle edit mode (enter/confirm)
                if (!s_settings_edit_mode) {
                    ...
                    settings_scroll_sync(); // ensure visible when entering edit
                } else {
